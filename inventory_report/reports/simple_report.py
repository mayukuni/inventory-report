...
class SimpleReport:
    ...
    def generate(products: list):
        ...
        valid_date = min([product["data_de_validade"] for product in products if product["data_de_validade"] > today])
        
        company, _ = Counter([product["nome_da_empresa"] for product in products]).most_common()[0]
